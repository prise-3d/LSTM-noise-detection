                ...
                for end in {50,100,150,200}; do
                    for sequence in {2,3,4,5,6,7,8,9,10}; do
                        ...
                        if [ $result -gt 0 ]; then
                            ...
                            python train_lstm_no_balancing.py --train data/datasets/${output_name_zones}/${output_name_zones}.train --test data/datasets/${output_name_zones}/${output_name_zones}.test --output ${output_name_zones}
                        fi
                    done
                done
            done
