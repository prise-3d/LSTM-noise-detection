...
command = "python display/display_thresholds_scene_file.py --params \"{0}\" --method {1} --model {2} --selected_zones {3} --scene {4} --thresholds {5} --seq_norm {6} --sequence {7} --save 1 --save_thresholds {8} --label_thresholds {9}"
...
for scene in sorted(os.listdir(p_folder)):    
    ...
    str_command = command.format(p_params, p_method, p_model, p_selected_zones, scene_path, p_thresholds, p_seqnorm, p_sequence, p_output, scene)
