...
def display_simulation_thresholds(scene, zones_predictions, humans, image_indices, output, n_predictions, nsamples, p_zone, p_threshold, every=None, zones_learned=None):
    ...
    for index_v, v in enumerate(predictions_data):
        ...
        predictions_label.append([0 if v < p_threshold else 1])
    ...
    plt.plot([ pthreshold for i in range(len(predictions))], lw=2, color='orange', linestyle='--')
...
def main():
    ...
    parser.add_argument('--pthreshold', type=float, help='sequence length expected')
    ...
    # 6. display results
    display_simulation_thresholds(scene_name, zones_predictions, human_thresholds, image_indices, p_output, p_n_prediction, p_nsamples, p_zone, p_pthreshold, p_every, zones_learned)
